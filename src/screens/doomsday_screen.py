    ...
    def _draw_stage_background(self, surface: pygame.Surface):
        ...
        if self.current_stage_theme == 1:
                        ...
                        if win_x + 6 <= building_x + building_width - 5:
                            ...
                            if (i + floor + window) % 3 == 0:
                                ...
                                if int(pygame.time.get_ticks() / 500) % 2 == 0 or (i + floor) % 4 != 0:
                                    ...
                                    pygame.draw.rect(surface, (60, 50, 30),  # Lit window
                                                  (win_x, win_y, 6, 8))
                            else:
                                pygame.draw.rect(surface, (15, 15, 15), 
                                              (win_x, win_y, 6, 8))
            ...
            for i in range(10):
                ...
                shadow_width = 8
                pygame.draw.rect(surface, (20, 18, 18),
                               (tower_x + tower_width - shadow_width, tower_y, 
                                shadow_width, tower_height))
                        ...
                        if win_x + 10 <= tower_x + tower_width - 8 and win_y + 12 <= tower_y + tower_height - 10:
                            if (i + floor * 3 + window) % 4 == 0:
                                ...
                                if int(pygame.time.get_ticks() / 300 + i) % 3 == 0:
                                    pygame.draw.rect(surface, (80, 20, 20),  # Red emergency light
                                                  (win_x, win_y, 10, 12))
                                else:
            ...
            for i in range(7):
                ...
                shadow_width = 12
                pygame.draw.rect(surface, (15, 12, 12),
                               (building_x + building_width - shadow_width, building_y,
                                shadow_width, building_height))
                
                # Structural damage - hole in building
                if i == 1 or i == 3:
                    hole_y = building_y + building_height // 3
                    hole_size = 25
                    pygame.draw.ellipse(surface, (30, 20, 20),
                                      (building_x + building_width//2 - hole_size//2,
                                       hole_y, hole_size, hole_size * 2))
                    ...
                    for window in range(min(4, max_windows)):
                        ...
                        if win_x + 12 <= building_x + building_width - 10 and win_y + 15 <= building_y + building_height - 15:
                            if (i * 7 + floor * 5 + window) % 5 == 0:
                                ...
                                if flicker == 0:
                                    pygame.draw.rect(surface, (60, 50, 30),  # Lit
                                                  (win_x, win_y, 12, 15))
                                else:
                                ...
                                if (floor * window) % 7 == 0:
                                    pygame.draw.line(surface, (15, 15, 15),
                                                   (win_x, win_y), (win_x + 12, win_y + 15), 1)
                            else:
                                pygame.draw.rect(surface, (8, 8, 8),
                                              (win_x, win_y, 12, 15))
            ...
            for x in range(0, SCREEN_WIDTH, 80):
                pygame.draw.line(surface, (40, 35, 35), 
                               (x, fence_y - 30), (x, fence_y + 30), 3)
            # Wire
            pygame.draw.line(surface, (50, 45, 45), 
                           (0, fence_y - 20), (SCREEN_WIDTH, fence_y - 20), 2)
            pygame.draw.line(surface, (50, 45, 45), 
                           (0, fence_y), (SCREEN_WIDTH, fence_y), 2)
            pygame.draw.line(surface, (50, 45, 45), 
                           (0, fence_y + 20), (SCREEN_WIDTH, fence_y + 20), 2)
            ...
            for i in range(3):
                ...
                pygame.draw.circle(glow_surface, (255, 100, 0, 60), 
                                 (glow_radius, glow_radius), glow_radius)
                surface.blit(glow_surface, (volcano_x - glow_radius, 
                                           volcano_y - volcano_height - glow_radius))
            ...
            for i in range(2):
                ...
                for j in range(3):
                    ...
                    glow_alpha = 100 - j * 30
                    glow_surface = pygame.Surface((glow_radius * 2, glow_radius * 2), pygame.SRCALPHA)
                    pygame.draw.circle(glow_surface, (255, 100, 0, glow_alpha), 
                                     (glow_radius, glow_radius), glow_radius)
                    surface.blit(glow_surface, (volcano_x - glow_radius, 
                                               volcano_y - volcano_height - glow_radius))
            ...
            for i in range(len(river_points) - 1):
                pygame.draw.line(surface, (100, 20, 0), 
                               river_points[i], river_points[i+1], 25)
            # Middle lava layer
            for i in range(len(river_points) - 1):
                pygame.draw.line(surface, (200, 50, 0), 
                               river_points[i], river_points[i+1], 20)
            # Bright inner flow
            for i in range(len(river_points) - 1):
                pygame.draw.line(surface, (255, 100, 0), 
                               river_points[i], river_points[i+1], 15)
            # Hot center line
            for i in range(len(river_points) - 1):
                pygame.draw.line(surface, (255, 200, 50), 
                               river_points[i], river_points[i+1], 5)
            ...
            for i in range(8):
                ...
                if 0 <= river_index < len(river_points) - 1:
                    ...
                    pygame.draw.ellipse(surface, (255, 255, 100),
                                      (int(streak_pos), int(y_at_pos) - 2, 20, 4))
            ...
            for pool_x, pool_y, pool_width, pool_height in lava_pool_positions:
                ...
                pygame.draw.ellipse(surface, (255, 100, 0), 
                                  (pool_x + 2, pool_y + 2, pool_width - 4, pool_height - 4))
            ...
            for stake_x, stake_y in tree_positions:
                ...
                pygame.draw.line(surface, (20, 10, 5), 
                               (stake_x, stake_y), (stake_x, stake_y - 40), 4)
                # Broken branches
                pygame.draw.line(surface, (20, 10, 5), 
                               (stake_x, stake_y - 30), (stake_x - 15, stake_y - 35), 2)
                pygame.draw.line(surface, (20, 10, 5), 
                               (stake_x, stake_y - 20), (stake_x + 10, stake_y - 28), 2)
            ...
            for ring in range(5):
                ...
                pygame.draw.ellipse(ring_surface, color, 
                                  (0, 0, ring_size * 4, ring_size * 2))
                surface.blit(ring_surface, 
                           (portal_x - ring_size * 2, portal_y - ring_size))
            ...
            for tentacle_x, tentacle_base_y in tentacle_positions:
                ...
                for segment in range(5):
                    ...
                    if segment_width > 0:
                        pygame.draw.circle(surface, (60, 30, 80), 
                                         (int(segment_x), segment_y), segment_width)
                        # Sucker detail
                        if segment % 2 == 0:
                            pygame.draw.circle(surface, (40, 20, 60), 
                                             (int(segment_x), segment_y), segment_width - 2)
            ...
            for building in range(13):
                ...
                # Side shadow for depth
                shadow_width = 6
                pygame.draw.rect(surface, (25, 20, 20),
                               (building_x + building_width - shadow_width, building_y,
                                shadow_width, building_height))
                ...
                if building % 2 == 1:
                    ...
                    hole_size = 20
                    pygame.draw.ellipse(surface, (50, 35, 35),
                                      (building_x + building_width//2 - hole_size//2,
                                       hole_y, hole_size, hole_size))
                    ...
                    for window in range(building_width // 18):
                        ...
                        if win_y < building_y + building_height - 15:
                            if (building + floor + window) % 3 != 0:
                                pygame.draw.rect(surface, (8, 5, 5),
                                              (win_x, win_y, 7, 10))
            ...
            for building in range(8):
                ...
                if building % 2 == 0:
                    ...
                    for hole_num in range(2):
                        ...
                        pygame.draw.ellipse(surface, (45, 30, 30),
                                          (int(hole_x - hole_size//2),
                                           hole_y, hole_size, int(hole_size * 1.5)))
                    ...
                    for window in range(building_width // 16):
                        ...
                        if win_y < building_y + building_height - 20:
                            if (building * floor + window) % 4 != 0:
                                pygame.draw.rect(surface, (5, 3, 3),
                                              (win_x, win_y, 10, 13))
                                # Cracks in some windows
                                if (floor + window) % 5 == 0:
                                    pygame.draw.line(surface, (10, 8, 8),
                                                   (win_x, win_y), (win_x + 10, win_y + 13), 1)
                ...
                if building in [1, 3]:
                    ...
                    for fire_y_offset in fire_floors:
                        ...
                        for flame in range(3):
                            ...
                            pygame.draw.ellipse(surface, (255, 150 + flame * 20, 0),
                                              (int(flame_x - flame_width//2), int(fire_y - flame_height), 
                                               int(flame_width), int(flame_height)))
            ...
            main_crack_positions = [
                [(50, SCREEN_HEIGHT), (80, SCREEN_HEIGHT - 60), (70, SCREEN_HEIGHT - 120), 
                 (100, SCREEN_HEIGHT - 180), (90, SCREEN_HEIGHT - 220)],
                [(350, SCREEN_HEIGHT), (370, SCREEN_HEIGHT - 70), (360, SCREEN_HEIGHT - 130), 
                 (380, SCREEN_HEIGHT - 190), (375, SCREEN_HEIGHT - 230)],
                [(700, SCREEN_HEIGHT), (720, SCREEN_HEIGHT - 50), (710, SCREEN_HEIGHT - 110), 
                 (730, SCREEN_HEIGHT - 160), (725, SCREEN_HEIGHT - 200)],
                [(900, SCREEN_HEIGHT), (920, SCREEN_HEIGHT - 80), (910, SCREEN_HEIGHT - 140), 
                 (925, SCREEN_HEIGHT - 180)]
            ]
